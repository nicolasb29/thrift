...
public class THttpClient extends TEndpointTransport {
  ...
  private RequestConfig getRequestConfig() {
    ...
    if (connectTimeout_ > 0) {
      requestConfig =
          RequestConfig.copy(requestConfig).setConnectionRequestTimeout(Timeout.ofMilliseconds(connectTimeout_)).build();
    }
  ...
  private ConnectionConfig getConnectionConfig() {
    ...
    if (readTimeout_ > 0) {
      connectionConfig = ConnectionConfig.copy(connectionConfig).setSocketTimeout(Timeout.ofMilliseconds(readTimeout_)).build();
    }
  ...
  private void flushUsingHttpClient() throws TTransportException {
    ...
    try {
      ...
      client.execute(this.host, post, response -> {
